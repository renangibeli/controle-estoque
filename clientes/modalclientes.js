...
addBtn.addEventListener('click', () => {
    ...
    if(inputId.value){
        dbToDo = axios.patch
        serverToUse = `${server}/clientes/${inputId.value}`
        actionToDo = 'atualizado'
    } else {
        dbToDo = axios.post
        serverToUse = `${server}/clientes`
    ...
    const {
        ...
        cpfValid,
        emailValid
    } = clientService.validateFieldsCliente(inputName.value, unmaskCpf, inputEmail.value)

    if (nameValid && cpfValid && emailValid) {
        dbToDo(`${serverToUse}`, {
        ...
        if (!emailValid) {
            clientService.addInvalidClass(inputEmail, labelEmail)
...
clearBtn.addEventListener('click', () => {
    ...
    clientService.removeInvalidClass(inputEmail, labelEmail)
})
